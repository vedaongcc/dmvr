...
class HMDB51Factory(video_dataset.BaseVideoDatasetFactory):
    ...
    def __init__(self, base_dir: str, subset: str = "train", split: int = 1):
        """Constructor of HMDB51Factory."""

        if subset not in HMDB51Factory._SUBSETS:
            raise ValueError(
                'Invalid subset "{}". The available subsets are: {}'.format(
                    subset, HMDB51Factory._SUBSETS
                )
            )

        if split not in HMDB51Factory._SPLITS:
            raise ValueError(
                'Invalid split "{}". The available splits are: {}'.format(
                    split, HMDB51Factory._SPLITS
                )
            )

        num_shards = self._NUM_SHARDS[subset]
        shards = [
            f"{subset}_{split}-{i:05d}-of-{num_shards:05d}" for i in range(num_shards)
        ]
